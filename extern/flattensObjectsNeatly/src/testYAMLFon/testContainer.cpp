...
TEST_CASE("test yaml serialization of std::vector with strings", "[yaml][std][vector][string][serialize]") {
...
TEST_CASE("test yaml deserialization of std::vector with strings", "[yaml][std][vector][deserialize]") {
    YAML::Node node;
    node[0] = "hallo";
    node[1] = "welt";
    node[2] = "!";
    auto data = fon::yaml::deserialize<std::vector<std::string>>(node);
